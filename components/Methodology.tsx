...
export default function Methodology() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-0 bottom-0 w-[420px] h-[320px] pointer-events-none select-none opacity-[0.03] -z-0">
      ...
      <div className="w-full max-w-5xl mx-auto relative z-10">
        ...
        {/* Subtitle */}
        <p className="text-slate-500 text-base sm:text-lg mb-16 max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          {steps.map((step) => (
            <div
              key={step.number}
              className="flex flex-col items-start p-6 rounded-2xl bg-slate-50/70 border border-slate-100/80 hover:bg-white hover:border-[#42aae1]/30 hover:shadow-lg transition-all duration-300 group"
            >
              {/* Number Circle Badge */}
              <div className="w-10 h-10 rounded-full bg-[#3f519f] group-hover:bg-[#42aae1] text-white font-bold text-sm flex items-center justify-center mb-6 shadow-md transition-colors duration-300">
                {step.number}
              </div>

              {/* Step Title */}
              <h3 className="text-xl font-bold text-slate-900 mb-2.5 font-heading group-hover:text-[#3f519f] transition-colors">
                {step.title}
              </h3>
