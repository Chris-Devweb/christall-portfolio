...
export default function About() {
  ...
  return (
    <section
      id="a-propos"
      className="relative py-24 sm:py-32 px-4 sm:px-6 bg-white overflow-hidden text-slate-900"
    >
      {/* Subtle Ambient Logo Watermark in Background */}
      <div className="absolute -left-24 -bottom-20 w-[480px] h-[370px] pointer-events-none select-none opacity-[0.035] -z-0">
      ...
      <div className="hidden xl:block absolute -right-6 top-12 w-44 h-44 pointer-events-none select-none opacity-40 animate-float-slow">
        <Image
          src="/losange.png"
      ...
      <div className="w-full max-w-5xl mx-auto relative z-10">
        ...
        {/* Title */}
        <h2 className="text-3xl sm:text-5xl md:text-5xl font-bold tracking-tight text-slate-900 leading-[1.2] mb-14 font-heading max-w-2xl">
          Moi c'est ChristΛll. Et je fais briller tes idées.
        </h2>

        {/* 2-Column Content Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
          {/* Left Column: Atelier Architecture Image Card */}
          <div className="lg:col-span-6 relative group">
            <div className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden shadow-[0_15px_35px_rgba(0,0,0,0.08)] border border-slate-100 bg-slate-100">
              <Image
              ...
              <div className="absolute bottom-5 left-5 z-10">
                <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/95 backdrop-blur-md border border-white/60 shadow-lg text-xs font-semibold text-slate-800">
          ...
          {/* Right Column: Bio & Core Pillars & Metrics */}
            ...
            <div className="flex flex-wrap gap-2.5 sm:gap-3 mb-10">
              {qualities.map((item) => {
                ...
                return (
                  <div
                    key={item.name}
                    className="flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-slate-200/80 bg-slate-50/70 hover:bg-slate-100/90 text-xs font-medium text-slate-700 transition-colors shadow-xs"
                  >
                    <IconComponent size={14} className="text-[#3f519f]" />
                    <span>{item.name}</span>
                  </div>
                );
              })}
